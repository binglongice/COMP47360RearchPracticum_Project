...
# Original individual hour prediction
...
#             print("prediction min", prediction_min)
...
# 24 hour prediction grouped by model
...
#                     predictions_by_hour[f'hour_{hour}'] = prediction[0]  # Get the prediction value for the hour

#                 all_model_predictions[f'model_{model_number}'] = predictions_by_hour

#             # Calculate the global min and max values
#             all_predictions = [prediction for predictions in all_model_predictions.values() for prediction in predictions.values()]
#             prediction_min = min(all_predictions)
#             prediction_max = max(all_predictions)

#             # Normalize each prediction array before adding it to all_model_predictions
#             for model_number in model_numbers:
#                 predictions_by_hour = all_model_predictions[f'model_{model_number}']
#                 for hour, prediction in predictions_by_hour.items():
#                     normalized_prediction = (prediction - prediction_min) / (prediction_max - prediction_min)
#                     all_model_predictions[f'model_{model_number}'][hour] = normalized_prediction.tolist()
